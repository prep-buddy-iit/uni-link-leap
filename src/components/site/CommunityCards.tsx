...
export function CommunityCards({ exam }: { exam: ExamKey }) {
  ...
  return (
    <section id="community" className="mx-auto max-w-7xl px-5 py-14 sm:py-16">
      <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-14">
        ...
        <div className="lg:col-span-7">
          <a href={c.whatsapp} target="_blank" rel="noreferrer"
            className="link-card flex flex-col rounded-3xl p-7 sm:p-8 text-white"
            ...
            <p className="mt-3 text-white">Daily study prompts, doubt threads, and mentor micro-tips.</p>
            <ul className="mt-5 space-y-2.5 border-t border-white/20 pt-5">
              {c.perks.map((perk) => (
                <li key={perk} className="flex items-start gap-2.5 text-sm text-white">
                  <Check className="mt-0.5 h-4 w-4 shrink-0" />
                  <span>{perk}</span>
                </li>
              ))}
            </ul>
            <span className="mt-6 inline-flex w-fit items-center gap-2 rounded-full bg-white px-5 py-2.5 font-semibold text-[color:var(--whatsapp)]">
